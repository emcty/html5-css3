...
var stemplate = '<%var len = data.length;%><div class="hc-tabs-btn hc-wkt"> <ul> <%for( var i = 0 ; i < len ; i ++ ){%><li <%if(i === 0){%>class="curr"<%}%> ><a href="javascript:;"><%= data[i].short_tit%></a></li> <%}%></ul> </div> <div class="hc-tabs-page"> <ul> <%for( var i = 0 ; i < len ; i ++ ){%><li <%if(i === 0){%>class="curr"<%}%>><div class="title"><%= data[i].title%></div> <div id="<%= data[i].idname%>" class="container"></div> <%if(data[i].merge){%> <div id="<%= data[i+1].idname%>" class="container"></div><%}%></li> <%}%></ul> </div>'
...
var _this = {
    ...
    hcRepaint:function(index){
        ...
        if(index ===4){
            //产品要求再加个饼图
            $("#biao5").highcharts(_this.settings["biao5"]);
        }
    },
